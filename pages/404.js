...
export default function FourOhFour({ dockElements, keywords }) {
	useEffect(() => {
		navbarToggle();
		cursorSetup();
		//setImageGalleries();
	...
	return (
		<>
			<AutoHead title="404" keywords={ keywords } />

			<Navbar />

			<main className="w-full h-screen flex absolute justify-center center bg-cover" style={{backgroundImage: "url('bg.jpeg')"}}>
				<div className="w-max flex flex-col items-center">
                    <h1 className="text-white text-9xl font-bold text-left font-interBold" style={{paddingTop: "33vh", textShadow: "6px 6px 12px rgba(0, 0, 0, .5)"}}>404</h1>
                    <Link href={server + "/"}>
